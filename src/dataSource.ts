...
export class DataSource extends Disposable {
	...
	public getConfig(repo: string, remotes: ReadonlyArray<string>): Promise<GitRepoConfigData> {
		return Promise.all([
			...
			const diffToolName = getConfigValue(consolidatedConfigs, GitConfigKey.DiffTool);
			const guiDiffToolName = getConfigValue(consolidatedConfigs, GitConfigKey.DiffGuiTool);
			return {
				config: {
					branches: branches,
					diffTool: {
